...
$(contact2_input).keyup(function () {
  ...
  if (contact2Value.length == contact2_input.attr("placeholder").length) {
    if (contact3_group.hasClass("hidden") == true) {
      add_third_contact.removeClass("hidden");
      $("#contact2_extra_info").addClass("hidden");
    }
    ...
    add_third_contact.addClass("hidden");
    contact3_group.addClass("hidden");
    $("#contact2_extra_info").removeClass("hidden");
  }
});
...
function quantityFunction() {
  var input_value = $("#donation_quantity_input").val();

  if ((organ_name_input.val() == "Lungs") && (input_value > 1)) {
    // if ((page_title.val() == "Create donation offer") || page_title.val() == "Update donation offer") {
      $("#donate_type").val(1);
      $("#contact2_priority").html("(required)");
      $("#contact2_priority").addClass("text-info");
...
function getValue(input) {
  ...
  if (input.attr('name') == 'type') {
    if (input.val() == "0") {
      var type_child = blood_group_input.val();
      if ((page_title.val() == "Create donation offer") || page_title.val() == "Update donation offer") {
        var html = ("Blood <br> <span class='text-dark'>Post Title:</span> I want to donate Blood [" +type_child+"].");
      }
      else{
        var html = ("Blood <br> <span class='text-dark'>Post Title:</span> I need Blood [" + type_child + "] donor.");
      }
      resultValue = html;
    }
    if (input.val() == "1") {
      // resultValue = "Organ";
      var type_child = organ_name_input.val();
      if ((page_title.val() == "Create donation offer") || page_title.val() == "Update donation offer") {
        var html = ("Organ <br> <span class='text-dark'>Post Title:</span> I want to donate Organ [" +type_child+"].");
      }
      else{
        var html = ("Organ <br> <span class='text-dark'>Post Title:</span> I need Organ [" + type_child + "] donor.");
      }
      resultValue = html;
    }
    if (input.val() == "2") {
      // resultValue = "Tissue";
      var type_child = tissue_name_input.val();
      if ((page_title.val() == "Create donation offer") || page_title.val() == "Update donation offer") {
        var html = ("Tissue <br> <span class='text-dark'>Post Title:</span> I want to donate Tissue [" +type_child+"].");
      }
      else{
        var html = ("Tissue <br> <span class='text-dark'>Post Title:</span> I need Tissue [" + type_child + "] donor.");
      }
      resultValue = html;
    }
  }
